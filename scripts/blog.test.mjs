...
function fixture(t) {
  ...
  fs.mkdirSync(path.join(directory,'_content/blog'),{recursive:true});
  fs.cpSync(path.join(root,'_content/blog'),path.join(directory,'_content/blog'),{recursive:true});
  t.after(()=>{
...
test('one Markdown file generates an article, overview and the correct category',t=>{
  const dir=fixture(t);
  fs.writeFileSync(path.join(dir,'_content/blog/articles/new-article.md'),article());
...
test('drafts stay out of listings and old generated pages are removed',t=>{
  const dir=fixture(t), file=path.join(dir,'_content/blog/articles/new-article.md');
